...
def test_pulse_schedule_aos_renamed_up(backend, worker_id, tmp_path):
    ...
    if backend == MEMORY_BACKEND:
        ...
        ids2 = open_ids(backend, "a", worker_id, tmp_path, version="3.26.0")
        ids2[ids_name].get()

        assert ids2[ids_name].ec.launcher[0].name == "test"
...
def test_pulse_schedule_aos_renamed_down(backend, worker_id, tmp_path):
    ...
    ids = open_ids(backend, "w", worker_id, tmp_path, version="3.26.0")
    ids_name = "pulse_schedule"
    ids[ids_name].ids_properties.homogeneous_time = IDS_TIME_MODE_HOMOGENEOUS
    ...
    if backend == MEMORY_BACKEND:
        ...
        ids2 = open_ids(backend, "a", worker_id, tmp_path, version="3.25.0")
        ids2[ids_name].get()
